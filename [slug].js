...
import { buildStandardsTable, buildPercentageTable, epley1RM } from "../calc";

const SITE_URL = "https://repmax-app.vercel.app";
    ...
    step: [
      {
        "@type": "HowToStep",
        name: "Set up",
        text: `Set up with ${exercise.equipment.toLowerCase()}, using a weight you can control for a full, clean rep.`,
      },
      {
        "@type": "HowToStep",
        name: "Execute the movement",
        text: exercise.formNote,
      },
      {
        "@type": "HowToStep",
        name: "Complete a full range of motion",
        text: `A rep only counts if it moves through the full range of motion for ${exercise.name.toLowerCase()} — partial reps inflate the number without reflecting real strength.`,
      },
    ],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: `${exercise.name} Calculator`,
        item: `${SITE_URL}/${exercise.slug}`,
      },
    ],
  };

  const percentageTable = exercise.isFlagship && quickAnswerWeight
    ? buildPercentageTable(quickAnswerWeight)
    : null;

  const workedExampleResult = exercise.workedExample
    ? Math.round(epley1RM(exercise.workedExample.weight, exercise.workedExample.reps))
    : null;

  const canonicalUrl = `${SITE_URL}/${exercise.slug}`;
    ...
    <>
      ...
      <section className="mt-8 rounded-card bg-card p-5 shadow-sm sm:p-7">
        ...
        <p className="mt-1 text-sm text-mute">
          Estimated 1RM by tier. Men's and women's standards both shown
          below — use the calculator above for your exact bodyweight.
        </p>
      ...
      {exercise.isFlagship && (
        <section className="mt-8 rounded-card bg-card p-5 shadow-sm sm:p-7">
          ...
          <p className="mt-3 text-sm text-ink">
            This calculator uses the Epley formula — one of the most widely
            cited methods for estimating a one-rep max from a real working
            set: <span className="font-mono">1RM ≈ weight × (1 + reps ÷ 30)</span>.
            It's an estimate, not a lab measurement — see our{" "}
            <Link href="/methodology" className="font-semibold text-flare">
              full methodology
            </Link>{" "}
            for where it works best and where it doesn't.
          </p>
        </section>
      )}

      {exercise.workedExample && (
        <section className="mt-8 rounded-card bg-card p-5 shadow-sm sm:p-7">
          <h2 className="font-display text-xl font-bold text-ink">
            Worked Example
          </h2>
          <p className="mt-3 text-sm text-ink">
            A {exercise.workedExample.bodyweight} lb lifter doing{" "}
            {exercise.name.toLowerCase()} with {exercise.workedExample.weight} lb
            for {exercise.workedExample.reps} reps gets an estimated 1RM of
            around <span className="font-semibold text-flare">{workedExampleResult} lb</span> —
            try those exact numbers in the calculator above to see it work.
          </p>
        </section>
      )}
      ...
      {percentageTable && (
        <section className="mt-8 rounded-card bg-card p-5 shadow-sm sm:p-7">
          <h2 className="font-display text-xl font-bold text-ink">
            Training Percentages of Your 1RM
          </h2>
          <p className="mt-1 text-sm text-mute">
            Based on a {quickAnswerWeight} lb estimated 1RM (180 lb,
            Intermediate) — useful for planning working sets, not just
            knowing your max.
          </p>
          <div className="mt-4 overflow-x-auto">
            <table className="w-full min-w-[280px] border-collapse text-sm">
              <thead>
                <tr className="border-b border-line text-left text-xs uppercase tracking-wide text-mute">
                  <th className="py-2 pr-3">% of 1RM</th>
                  <th className="py-2">Weight</th>
                </tr>
              </thead>
              <tbody>
                {percentageTable.map((row) => (
                  <tr key={row.percent} className="border-b border-line/60">
                    <td className="py-2 pr-3 font-semibold text-ink">{row.percent}%</td>
                    <td className="py-2 text-ink">{row.weight} lb</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {exercise.averageSection && (
        <section className="mt-8 rounded-card bg-card p-5 shadow-sm sm:p-7">
          <h2 className="font-display text-xl font-bold text-ink">
            {exercise.averageSection.title}
          </h2>
          <p className="mt-3 text-sm text-ink">{exercise.averageSection.body}</p>
        </section>
      )}

      {exercise.repRangeGuidance && (
        <section className="mt-8 rounded-card bg-card p-5 shadow-sm sm:p-7">
          <h2 className="font-display text-xl font-bold text-ink">
            {exercise.repRangeGuidance.title}
          </h2>
          <p className="mt-3 text-sm text-ink">{exercise.repRangeGuidance.body}</p>
        </section>
      )}
      ...
      {exercise.comparisonSection && comparisonTarget && (
        <section className="mt-8 rounded-card bg-card p-5 shadow-sm sm:p-7">
          ...
          >
            Compare your {exercise.category.toLowerCase()} strength on the {comparisonTarget.name} calculator →
          </Link>
        </section>
      )}
